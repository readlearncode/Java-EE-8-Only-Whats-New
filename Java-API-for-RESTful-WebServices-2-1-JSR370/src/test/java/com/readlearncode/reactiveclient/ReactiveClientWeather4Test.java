 ...
 * @version 1.0
 */
public class ReactiveClientWeather4Test {
    ...
    public void reactive() throws InterruptedException {
        ...
        // Stream of cities
        Stream.of("London", "Paris", "New York", "Tokyo", "Kandor")
                // Stage 1: Retrieve a list of city ids (woeid) for each city
                .map(city -> location.resolveTemplate("city", city).request().rx().get(Location.class)
                        .exceptionally(t -> new Location(city)))
                .forEach(locationStage -> locationStage.thenCompose(location -> {

                    // Stage 2: Then for each city id retrieve the weather forecast
                    CompletionStage<Weather> weatherResult =
                            weather.resolveTemplate("woeid", location.getWoeid()).request().rx().get(Weather.class)
                                    .exceptionally(t -> new Weather(location.getTitle(), -273D));

                    // Stage 3: Compose the weather data for that city
                    //noinspection unchecked
                    return CompletableFuture
                            .completedFuture(new CityData(location))
                            .thenCombine(weatherResult, CityData::weather);

                    // Stage 4: After each completion, print to the console the city and weather data
